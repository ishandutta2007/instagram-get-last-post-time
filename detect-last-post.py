...
class InstagramScraper:
    ...
    def profile_page_recent_posts_time(self, profile):
        ...
        try:
            ...
            time_difference = datetime.now() - datetime.fromtimestamp(
                int(results[0]["taken_at_timestamp"])
            )
    ...
    def tag_page_media(self, tag):
        ...
        try:
            ...
            total_medias = json_data["entry_data"]["TagPage"][0]["graphql"]["hashtag"][
                "edge_hashtag_to_media"
            ]["count"]
            print("Total Medias: {}".format(total_medias))
            medias = json_data["entry_data"]["TagPage"][0]["graphql"]["hashtag"][
                "edge_hashtag_to_media"
            ]["edges"]
            ...
            top_posts = json_data["entry_data"]["TagPage"][0]["graphql"]["hashtag"][
    ...
    def get_min_likes_from_tag_page_media(self, tag):
        ...
        try:
            ...
            for ctr, node in enumerate(top_posts):
                ...
                tot_top_likes += node["edge_media_preview_like"]["count"]
                top_likes_list.append(node["edge_media_preview_like"]["count"])
            print("tot_media_likes({}): {}".format(len(medias), tot_media_likes))
            print("tot_top_likes({}): {}".format(len(top_posts), tot_top_likes))
            print("top_likes_list: {}".format(top_likes_list))
            print("====xxx===")
            if len(top_likes_list) > 0:
                print("min(top_likes_list): {}".format(min(top_likes_list)))
                return min(top_likes_list)
...
def main():
    ...
    for tag in tags:
        obj.get_min_likes_from_tag_page_media(tag)
